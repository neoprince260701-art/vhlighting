...
export default function Page() {
  ...
  const searchResults = useMemo(() => {
    ...
    if (!q) return [];
    return products.filter(x => x.sku.toLowerCase().includes(q) || x.name.toLowerCase().includes(q) || (x.light_color||"").toLowerCase().includes(q)).slice(0, 20);
  ...
  function addProduct(product: Product) {
    setItems(current => {
      ...
      return [...current, { product_id: product.id, sku: product.sku, product_name: product.name, attribute: product.light_color || null, unit: product.unit, quantity: 1, unit_price: Number(product.price), line_total: Number(product.price) }];
    });
    setProductQuery("");
  ...
  async function save() {
    ...
    const { data: sameDayOrders } = await supabase.from("orders").select("order_no").ilike("order_no", `${prefix}%`);
    const nextSequence = Math.max(0, ...(sameDayOrders || []).map(x => Number(x.order_no.split("-").pop()) || 0)) + 1;
    let orderNo = `${prefix}${String(nextSequence).padStart(3, "0")}`;
    let order: any = null;
    let error: any = null;
    for (let attempt = 0; attempt < 3; attempt++) {
      const result = await supabase.from("orders").insert({ order_no: orderNo, customer_id: customer?.id || null, customer_name: customer?.name || "Khách lẻ", customer_phone: customer?.phone || null, customer_address: customer?.address || null, subtotal, discount, shipping_fee: shippingFee, total, status: "Mới", note }).select().single();
      order = result.data; error = result.error;
      if (!error) break;
      if (error.code !== "23505") break;
      orderNo = `${prefix}${String(nextSequence + attempt + 1).padStart(3, "0")}`;
    }
    if (error || !order) { alert(error?.message || "Không thể tạo đơn hàng"); setBusy(false); return; }
    ...
    <div className="panel order-editor">
      <div className="order-top-grid">
        ...
        <div className="product-picker"><label>Nhập mã hoặc tên sản phẩm</label><div className="search-box large"><Search size={18}/><input value={productQuery} placeholder="Ví dụ: VH-AT hoặc âm trần…" onFocus={() => setShowResults(true)} onChange={e => { setProductQuery(e.target.value); setShowResults(true); }}/></div>
          {showResults && productQuery && <div className="product-results">{searchResults.map(x => <button key={x.id} type="button" onClick={() => addProduct(x)}><div><span className="sku-pill">{x.sku}</span><b>{x.name}</b>{x.light_color&&<span className="light-pill">{x.light_color}</span>}<small>{x.unit} · Tồn {x.stock}</small></div><strong>{Number(x.price).toLocaleString("vi-VN")} ₫</strong><Plus size={17}/></button>)}{!searchResults.length && <div className="empty-state">Không có sản phẩm phù hợp</div>}</div>}
        </div>
      </div>

      <div className="table-wrap order-table"><table><thead><tr><th>STT</th><th>Mã nhóm</th><th>Tên sản phẩm</th><th>Thuộc tính</th><th>ĐVT</th><th>SL</th><th>Đơn giá</th><th>Thành tiền</th><th></th></tr></thead><tbody>
        {items.map((x, i) => <tr key={x.product_id || i}><td>{i + 1}</td><td><span className="sku-pill">{x.sku}</span></td><td><b>{x.product_name}</b></td><td>{x.attribute||"—"}</td><td>{x.unit}</td><td><input className="mini" type="number" min="1" value={x.quantity} onChange={e => patch(i, "quantity", Number(e.target.value))}/></td><td><input className="money" type="number" min="0" value={x.unit_price} onChange={e => patch(i, "unit_price", Number(e.target.value))}/></td><td className="number-cell"><b>{x.line_total.toLocaleString("vi-VN")} ₫</b></td><td><button className="icon-danger" onClick={() => setItems(v => v.filter((_, n) => n !== i))}><Trash2 size={16}/></button></td></tr>)}
        {!items.length && <tr><td colSpan={9} className="empty-state">Nhập mã sản phẩm phía trên để thêm vào đơn.</td></tr>}
      </tbody></table></div>
