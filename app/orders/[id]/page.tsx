...
export default function Page() {
  ...
  return <AuthGuard><AppShell title="Chi tiết đơn hàng" action={<button className="primary" onClick={() => window.print()}>In / Lưu PDF A4</button>}>
    {!order ? <div>Đang tải…</div> : <article className="invoice invoice-final-11">
      ...
      <table className="invoice-table invoice-table-final"><thead><tr><th>STT</th><th>Tên sản phẩm</th><th>Thuộc tính</th><th>ĐVT</th><th>SL</th><th>Đơn giá</th><th>Thành tiền</th></tr></thead><tbody>
        {items.map((x,i)=><tr key={x.id||i}><td>{i+1}</td><td>{x.product_name}</td><td>{x.light_color||""}</td><td>{x.unit}</td><td>{x.quantity}</td><td>{money(x.unit_price)}</td><td>{money(x.line_total)}</td></tr>)}
      </tbody></table>
      ...
      <section className="invoice-notes invoice-notes-single"><div><b>Ghi chú đơn hàng:</b><p>{order.note||""}</p></div></section>
      <section className="sign sign-two"><div><b>Người lập phiếu</b><span>(Ký và ghi rõ họ tên)</span><strong>{company.invoice_creator_name||"Vũ Lighting"}</strong></div><div><b>Khách hàng</b><span>(Ký và ghi rõ họ tên)</span></div></section>
