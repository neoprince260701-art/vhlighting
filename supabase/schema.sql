...
create table if not exists public.products(id uuid primary key default gen_random_uuid(),sku text not null,name text not null,attribute text,unit text not null default 'Cái',price numeric(14,2) not null default 0,stock integer not null default 0,created_at timestamptz not null default now());
...
create table if not exists public.company_settings(
  ...
  bank_holder text, bank_branch text, logo_url text, warranty_note text, invoice_footer text, invoice_creator_name text default 'Vũ Lighting',
