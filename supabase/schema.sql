...
create table if not exists public.products(id uuid primary key default gen_random_uuid(),sku text not null,name text not null,light_color text,unit text not null default 'Cái',price numeric(14,2) not null default 0,stock integer not null default 0,created_at timestamptz not null default now());
...
create table if not exists public.order_items(id uuid primary key default gen_random_uuid(),order_id uuid not null references public.orders(id) on delete cascade,product_id uuid references public.products(id) on delete set null,sku text not null,product_name text not null,attribute text,unit text not null,quantity numeric(12,2) not null default 1,unit_price numeric(14,2) not null default 0,line_total numeric(14,2) not null default 0);
...
create table if not exists public.company_settings(
  ...
  bank_holder text, bank_branch text, logo_url text, warranty_note text, invoice_footer text, invoice_creator_name text not null default 'Vũ Lighting',
...
insert into public.company_settings(id,company_name,tagline,hotline,website,email,warranty_note,invoice_footer,invoice_creator_name)
values(1,'VH LIGHTING','Chuyên cung cấp thiết bị chiếu sáng','0877 933 362','vulighting.com','vat.vuhoanglighting@gmail.com','Sản phẩm được bảo hành theo chính sách của nhà sản xuất. Vui lòng giữ phiếu bán hàng để được hỗ trợ.','Cảm ơn Quý khách đã tin tưởng và lựa chọn VH Lighting.','Vũ Lighting')
